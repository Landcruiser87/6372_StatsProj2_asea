...
Let's look at the VIFs to confirm collinear variables.
```{r}
library(ResourceSelection)
model.main<-glm(Obese_Class ~ ., data=data2,family = binomial(link="logit"))
### NOT WORKING (vif(model.main)[,3])^2
```
...
```{r Variance}
round(cov(data2[2:12]), 2) 
sum(diag(cov(data2[2:12]))) # total variance
```
