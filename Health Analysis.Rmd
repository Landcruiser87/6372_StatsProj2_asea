...
# Model
logreg.fit <- train(Obese_Class ~ ., data=logreg.train, method="glm", family=binomial(),trControl=control)
logreg.fit

# predict on test
logreg.pred <- predict(logreg.fit,logreg.test)

# results
confusionMatrix(logreg.pred,logreg.test$Obese_Class)
...
# Model
logreg.cmplxfit <- train(Obese_Class ~ ., data=train, method="glm", family=binomial(),trControl=control)
logreg.cmplxfit

# predict on test
logreg.cmplxpred <- predict(logreg.cmplxfit,test)

# results
confusionMatrix(logreg.cmplxpred,test$Obese_Class)
```
...
#library(klaR)
#partimat(Obese_Class~., data=train, method="qda")
...
rf_default <- train(Obese_Class~., data=data, method="rf", metric="Accuracy", tuneGrid=tunegrid, trControl=control)
print(rf_default)
...
# KNN NOT WORKING, PROD DUE TO ARRAY INDEXES
#knn<-FNN::knn.reg(train = train[,-c(3,4)], test =test[,-c(3,4)], y = train$Smokers, k = 5)
#testMSE.knn<-mean( ( test$Smokers-knn$pred)^2)
...
The OLS method has the lowest MSE of 1.823254.
