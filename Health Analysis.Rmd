...
## if we are not using Premature_Death_Rate, we can remove
...
## Can use if we want.  Doesn't matter to jebus. Inputting here for now.
#library(purrr)
#library(tidyr)
#library(ggplot2)
#
#plotAllNumeric <- function(predictors){
#   df%>%keep(is.numeric) %>%
#   gather() %>%
#   ggplot(aes(value)) +
...
```{r Remove Outliers}
data <- data[!rowSums(data[c(-1:-15,-17:-24)] > 20),] # removed unemployment outliers for migrant farming counties
data <- data[!rowSums(data[c(-1:-16,-18:-24)] > 50),] # removed unemployment outliers for migrant farming counties
```
...
glm.pred1 <-predict(glm.fit, newdata = train, type = "response")
obsv.class <- (test$Obese_Class)
glm_cm<-table(glm.pred1,obsv.class)
glm_cm

#logit overall misclassification rrror rate
glm_ME<-(glm_cm[2,1]+glm_cm[1,2])/(glm_cm[1,2]+glm_cm[2,2])
glm_ME

#logit Overall Accuracy
1-glm_ME
```
...
The variables are all in percentage form (without dividing by 100). Is is fortunate to already have this type of normalization. It reduces/eliminates the scale sensitivity seen with PCA. Otherwise, we could have exaggerated influence or bias from variables with large variances. 
